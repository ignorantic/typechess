...
import {
  getAttackedSquares, isCastling, isSquare, willBeCheckmate, willBeInCheck,
} from './utils';
import FEN from './FEN';

class Notation {
  public static toUCI(start: Square, stop: Square, promType: PieceType = null): string | null {
    if (
      !start || !stop
      || !isSquare(start.file, start.rank)
      || !isSquare(stop.file, stop.rank)
    ) return null;
    const startUCI = Notation.squareToUCI(start.file, start.rank);
    const stopUCI = Notation.squareToUCI(stop.file, stop.rank);
    const pieces = ['', 'r', 'n', 'b', 'q'];
    const pieceTypeUCI = (promType && pieces[promType]) || '';
    return `${startUCI}${stopUCI}${pieceTypeUCI}`;
  }

  public static squareToUCI(file: number, rank: number) {
  ...
  public static UCIToSquare(move: string): Square {
    const shiftFile = 97;
    const shiftRank = 1;
  ...
  public static UCIToAN(fen: string, move: string, pieces: string[]) {
    const { board, turn } = FEN.parse(fen);
    const startSquare: Square = Notation.UCIToSquare(move.slice(0, 2));
    const stopSquare: Square = Notation.UCIToSquare(move.slice(2, 4));
  ...
  public static UCIToSAN(fen: string, move: string) {
    const pieces = ['', 'r', 'n', 'b', 'q', 'k'];
    return Notation.UCIToAN(fen, move, pieces);
  }

  public static UCIToFAN(fen: string, move: string): string {
    const pieces = ['', '\u265C', '\u265E', '\u265D', '\u265B', '\u265A'];
    return Notation.UCIToAN(fen, move, pieces);
  }
}
